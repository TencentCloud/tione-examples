...
from dataset.AdvertiseGenDataset.build_dataset import buid_dataset_train, buid_dataset_eval, DataCollatorForSupervisedDataset
### TIACC
...
### TIACC eval
from tilearn_seq2seq_trainer import TilearnSeq2SeqTrainer as Trainer
...
class PrintCUDAINFOCallback(transformers.TrainerCallback):
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        ...
        result = [float(size) for size in subprocess.check_output(["nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits"], shell=True).decode("utf-8")[:-1].split("\n")]
        os.environ["CUDA_MEMORY_MAX"] = str(max(max(result), float(os.getenv('CUDA_MEMORY_MAX', "0"))))
        os.environ["CUDA_MEMORY_AVG"] = str(max(sum(result)/len(result), float(os.getenv('CUDA_MEMORY_AVG', "0"))))
        CUDAINFO = {
            "CUDA_MEMORY_MAX": float(os.environ["CUDA_MEMORY_MAX"]),
            "CUDA_MEMORY_AVG": float(os.environ["CUDA_MEMORY_AVG"]),
        }
...
def main():
    ...
    config_kwargs = {
        ...
        "trust_remote_code": True if model_args.trust_remote_code else False,
    }
    ...
    if training_args.do_train:
        with training_args.main_process_first(desc="loading and tokenization"):
            ...
            logger.info(f"train files: {' '.join(files)}")
            train_dataset = buid_dataset_train(
                data_path=files, 
                tokenizer=tokenizer, 
    ...
    if training_args.do_eval:
        with training_args.main_process_first(desc="loading and tokenization"):
            ...
            files = data_args.validation_file
            logger.info(f"eval files: {' '.join(files)}")
            eval_dataset = buid_dataset_eval(
                data_path=files, 
                tokenizer=tokenizer, 
                max_seq_length=data_args.max_seq_length,
                data_cache_dir = None, 
    ...
    trainer.add_callback(PrintCUDAINFOCallback)
